...
// MmapModel maps a model file into memory using mmap on macOS
...
func MmapModelAtOffset(fd int, offset int64, size int) (addr uintptr, data []byte, err error) {
	...
	fmt.Printf("DEBUG: MmapModelAtOffset - offset: %d, pageAlignedOffset: %d, adjustment: %d, mapSize: %d\n",
		offset, pageAlignedOffset, adjustment, mapSize)
	...
	// Get the address of the actual model data (after adjustment)
	modelAddr := uintptr(unsafe.Pointer(&mappedData[0])) + uintptr(adjustment)

	// IMPORTANT: Return the full mapped data for proper unmapping
	// But the address points to the adjusted location
	return modelAddr, mappedData, nil
}
...
func LoadSelfContainedModel(offset int64, size int) (addr uintptr, data []byte, err error) {
	...
	// On macOS, always use memory loading instead of mmap to avoid llama.cpp mmap issues
	if runtime.GOOS == "darwin" {
		fmt.Printf("macOS detected, using memory loading instead of mmap\n")
		...
		fmt.Printf("Successfully loaded self-contained model into memory (%.2f MB)\n",
	...
	// Use the mapped data slice directly instead of unsafe pointer conversion
	fmt.Printf("Debug: First 16 bytes of model data at 0x%x: ", mappedAddr)
